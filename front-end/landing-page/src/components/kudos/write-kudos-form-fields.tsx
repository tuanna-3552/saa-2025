    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "row" }}>
        ...
        <div style={{ flex: 1, height: "40px", border: BD, borderRadius: "0 8px 0 0", display: "flex", alignItems: "center", justifyContent: "flex-end", padding: "0 16px", boxSizing: "border-box" }}>
          <span style={{ ...FM, fontSize: "16px", lineHeight: "24px", color: "#E46060", letterSpacing: "0.15px" }}>{t("kudos.formFields.communityStandards")}</span>
        </div>
      </div>
      <textarea value={content} onChange={(e) => onChange(e.target.value)}
        placeholder={t("kudos.formFields.contentPlaceholder")}
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <span style={{ ...FM, fontSize: "16px", lineHeight: "24px", color: TP, letterSpacing: "0.5px" }}>{t("kudos.formFields.mentionHint")}</span>
        ...
        {hashtags.map((tag, i) => (
          <div key={i} style={{ border: BD, background: WH, borderRadius: "8px", padding: "8px 8px 8px 16px", display: "flex", alignItems: "center", gap: "8px", height: "48px" }}>
            ...
            <button type="button" onClick={() => onRemove(i)} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex" }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/kudos/icons/close-tiny.svg" alt={t("kudos.formFields.removeAlt")} width={17} height={17} />
            </button>
          </div>
        ))}
        {hashtags.length < 5 && (
          <div style={{ border: BD, background: WH, borderRadius: "8px", padding: "4px 8px", display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "2px", minHeight: "48px", justifyContent: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
              ...
              <input type="text" value={input} onChange={(e: ChangeEvent<HTMLInputElement>) => onInputChange(e.target.value)} onKeyDown={handleKey} placeholder={t("kudos.formFields.hashtagPlaceholder")}
                style={{ border: "none", outline: "none", background: "transparent", ...FM, fontSize: "11px", color: TS, width: "64px" }} />
            </div>
            <span style={{ ...FM, fontSize: "11px", lineHeight: "16px", color: TS, letterSpacing: "0.5px" }}>{t("kudos.formFields.maxFive")}</span>
          </div>
        )}
      </div>
    </div>
        ...
        {images.length < 5 && (
          <button type="button" onClick={onAdd}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/kudos/icons/plus.svg" alt="" aria-hidden width={24} height={24} />
              <span style={{ ...FM, fontSize: "11px", color: TS }}>{t("kudos.formFields.image")}</span>
            </div>
            <span style={{ ...FM, fontSize: "11px", lineHeight: "16px", color: TS, letterSpacing: "0.5px" }}>{t("kudos.formFields.maxFive")}</span>
          </button>
        )}
