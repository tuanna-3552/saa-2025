 ...
 * Rendering is config-driven via statuslineLayout in .sk.json.
...
const { loadConfig, readSessionState } = require('./hooks/lib/sk-config-utils.cjs');
...
// Read stdin with optional inactivity timeout (SK_STATUSLINE_STDIN_TIMEOUT_MS)
async function readStdin() {
  return new Promise((resolve, reject) => {
    const chunks = [];
    stdin.setEncoding('utf8');
    const parsedTimeout = Number.parseInt(env.SK_STATUSLINE_STDIN_TIMEOUT_MS || '', 10);
...
async function main() {
  try {
    ...
    if (sessionId && contextSize > 0) {
      try {
        const contextDataPath = path.join(os.tmpdir(), `sk-context-${sessionId}.json`);
