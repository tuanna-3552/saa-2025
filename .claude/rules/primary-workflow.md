...
- Delegate to `doc-writer` agent to update docs in `./docs` directory if any.
...
- Use `/tkm:preview-output --explain <topic>` to generate visual explanation with ASCII + Mermaid
- Use `/tkm:preview-output --diagram <topic>` for architecture and data flow diagrams
- Use `/tkm:preview-output --slides <topic>` for step-by-step walkthroughs
- Use `/tkm:preview-output --ascii <topic>` for terminal-friendly output only
