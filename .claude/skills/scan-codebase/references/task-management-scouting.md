...
Scout tasks are **independent** from takumi/planning phase tasks — NOT parent-child.

**Rationale:** Different lifecycle. Scout completes before takumi continues. Mixing creates confusion in TaskList.

**Sequence when takumi spawns scout:**
1. Takumi Step 2 → spawns planner → planner spawns scout
2. Scout registers its own tasks (Step 3), executes (Step 4-5)
3. Scout returns aggregated report → planner continues
4. Takumi Step 3 hydrates phase tasks (separate from scout tasks)
