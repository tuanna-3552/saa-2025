...
function Install-NodeDeps {
    ...
    $automateBrowserPath = Join-Path $ScriptDir "automate-browser\scripts"
    $automateBrowserPackageJson = Join-Path $automateBrowserPath "package.json"
    if ((Test-Path $automateBrowserPath) -and (Test-Path $automateBrowserPackageJson)) {
        Write-Info "Installing automate-browser dependencies..."
        Push-Location $automateBrowserPath
        npm install --quiet
        Pop-Location
        Write-Success "automate-browser dependencies installed"
    }

    # think-sequential
    $seqThinkingPath = Join-Path $ScriptDir "think-sequential"
    $seqPackageJson = Join-Path $seqThinkingPath "package.json"
    if ((Test-Path $seqThinkingPath) -and (Test-Path $seqPackageJson)) {
        Write-Info "Installing think-sequential dependencies..."
        Push-Location $seqThinkingPath
        npm install --quiet
        Pop-Location
        Write-Success "think-sequential dependencies installed"
    }

    # manage-mcp
    $manageMcpPath = Join-Path $ScriptDir "manage-mcp\scripts"
    $manageMcpPackageJson = Join-Path $manageMcpPath "package.json"
    if ((Test-Path $manageMcpPath) -and (Test-Path $manageMcpPackageJson)) {
        Write-Info "Installing manage-mcp dependencies..."
        Push-Location $manageMcpPath
        npm install --quiet
        Pop-Location
        Write-Success "manage-mcp dependencies installed"
    }
    ...
    $viewPlansPath = Join-Path $ScriptDir "view-plans"
    $viewPlansPackageJson = Join-Path $viewPlansPath "package.json"
    if ((Test-Path $viewPlansPath) -and (Test-Path $viewPlansPackageJson)) {
        Write-Info "Installing view-plans dependencies..."
        Push-Location $viewPlansPath
        npm install --quiet
        Pop-Location
        Write-Success "view-plans dependencies installed"
    }
}
...
function Test-Installations {
    ...
    $npmPackages = @("pnpm", "wrangler", "repomix")
    foreach ($package in $npmPackages) {
