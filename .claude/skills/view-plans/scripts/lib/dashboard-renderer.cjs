...
function generateCardMeta(plan) {
  ...
  // Issue tag - clickable link to GitHub (uses branch to derive repo, falls back to takumi-agent-kit)
  if (plan.issue) {
    // TODO: Make repo configurable via project settings
    const issueUrl = `https://github.com/takumi-agent-kit/takumi-agent-kit/issues/${plan.issue}`;
