...
description: Forge faithful interfaces from blueprints — replicate designs, screenshots, or videos into polished frontend code. Web components, 3D experiences, quick prototypes, immersive interfaces. Every detail matched, no shortcuts.
...
A blueprint is not the object. The object must be forged from the blueprint — with faithfulness to the original vision, precision in every measurement, and resistance to shortcuts that would betray the design.

This skill translates visual designs into production-grade frontend code. Real working code. Exceptional attention to aesthetic fidelity. No generic defaults. No "AI slop."
...
**All workflows**: Activate `tkm:design-ui` skill FIRST for design intelligence.

**Precedence:** When anti-slop rules (below) conflict with `tkm:design-ui` recommendations (e.g., Inter font, AI Purple palette, Lucide-only icons), substitute with alternatives from `./references/anti-slop-rules.md` unless the user explicitly requested the conflicting choice.

## Screenshot/Video Replication (Quick Reference)

1. **Analyze** the screenshot/video directly — Claude vision extracts colors, fonts, spacing, effects
