...
Entry point for using the `sk:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `sk:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `sk:frontend-design` with `sk:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `sk:chrome-devtools` Skill
Use `sk:chrome-devtools` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `sk:media-processing` Skill
