...
Complete workflow for generating design-aligned visual assets using `sk:ai-multimodal` skill.
...
Use `sk:ai-multimodal` skill's image generation capabilities:
...
Use `sk:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `sk:media-processing` skill for post-generation adjustments
