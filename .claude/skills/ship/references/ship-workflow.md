...
2. Delegate to `reviewer` subagent with the diff
3. Two-pass model:
...
1. Invoke `/tkm:write-journal` skill via `journal-writer` subagent in background:
...
1. Invoke `/tkm:manage-docs update` skill via `doc-writer` subagent in background:
