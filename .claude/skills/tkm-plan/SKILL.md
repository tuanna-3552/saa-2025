...
description: "Create a detailed implementation plan before writing code — design architecture, break work into phases, generate executable technical roadmaps. Activate before any significant feature or system implementation."
...
The master craftsman draws before cutting.
A blueprint is not bureaucracy — it is the difference between a piece that holds and one that splinters.

Create detailed technical implementation plans through research, codebase analysis, solution design, and comprehensive documentation. The blueprint is what `/tkm:takumi` executes — draft it with care.
...
If invoked with a task description, proceed with the blueprint workflow. If invoked WITHOUT arguments or with unclear intent, use `AskUserQuestion` to present available operations:

| Operation | Description |
|-----------|-------------|
| `(default)` | Draft implementation blueprint for a task |
| `archive` | Write session record & archive plans |
| `red-team` | Adversarial blueprint review |
| `validate` | Critical questions interview |

Present as options via `AskUserQuestion` with header "Blueprint Operation", question "What would you like to do?".

## Blueprint Modes

Default: `--auto` (analyze task complexity and auto-pick mode).

| Flag | Mode | Research | Red Team | Validation | Takumi Flag |
|------|------|----------|----------|------------|-------------|
...
5. **Blueprint Documentation** → Write comprehensive plan via planner subagent
6. **Red Team Review** → Run `/tkm:create-plan red-team {plan-path}` (hard/parallel/two modes)
7. **Post-Blueprint Validation** → Run `/tkm:create-plan validate {plan-path}` (hard/parallel/two modes)
8. **Hydrate Tasks** → Create Claude Tasks from phases (default on, `--no-tasks` to skip)
9. **Context Reminder** → Output takumi command with absolute path (MANDATORY)
10. **Record Session** → Run `/tkm:write-journal` to write a concise session record upon completion

## Output Requirements

**IMPORTANT:** Invoke "/tkm:organize-files" skill to organize the outputs.

- DO NOT implement code — only create blueprints
...
Blueprint files = persistent. Tasks = session-scoped. Hydration bridges the gap.

**Default:** Auto-hydrate tasks after blueprint files are written. Skip with `--no-tasks`.
**3-Task Rule:** <3 phases → skip task creation.
**Fallback:** Task tools (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`) are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for tracking. Blueprint files remain the source of truth; hydration is an optimization, not a requirement.

Load: `references/task-management.md` for hydration pattern, TaskCreate patterns, takumi handoff protocol.
...
5. Takumi picks up via TaskList (same session) or re-hydrates (new session)
...
After creating blueprint: `node .claude/scripts/set-active-plan.cjs {plan-dir}`
...
|------------|-----------|---------|
| `/tkm:create-plan archive` | `references/archive-workflow.md` | Archive plans + write session records |
| `/tkm:create-plan red-team` | `references/red-team-workflow.md` | Adversarial blueprint review with hostile reviewers |
| `/tkm:create-plan validate` | `references/validate-workflow.md` | Validate blueprint with critical questions interview |
...
**Remember:** Blueprint quality determines forge quality. A weak plan produces weak work. Draft with care.
