...
3. If no plan found → Ask user to specify path or run `/tkm:create-plan` first
...
Launch reviewers simultaneously via Task tool with `subagent_type: "reviewer"`.
...
Remind user to run `/tkm:create-plan validate` then `/tkm:takumi --auto`.
