...
**Why `--auto` takumi flag?** Fast planning pairs with fast execution — skip review gates.
...
2. Read codebase docs; if stale/missing: run `/tkm:scan-codebase` to search codebase
...
**Why no takumi flag?** Thorough planning needs interactive review gates.
...
7. **Context reminder:** `/tkm:takumi --parallel {absolute-plan-path}/plan.md`
...
| Mode | Takumi Command |
|------|-----------------------------|
| fast | `/tkm:takumi --auto {path}/plan.md` |
| hard | `/tkm:takumi {path}/plan.md` |
| parallel | `/tkm:takumi --parallel {path}/plan.md` |
| two | `/tkm:takumi {path}/plan.md` |

> **Best Practice:** Run `/clear` before implementing to reduce planning-context carryover.
> Then run the takumi command above.
