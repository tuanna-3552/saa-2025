---
name: tkm:agent-team
description: "Assemble a guild of specialist agents for parallel multi-session work — each agent owns their domain, all forging the same commission. Use for research, implementation, review, and debug workflows that demand true independence between teammates."
...
# Coordinating the Guild

No single craftsman can master all disciplines; the guild's strength is in knowing who does what, and trusting them to do it. When the commission exceeds what one pair of hands can accomplish alone, the master coordinates — assigning work, setting boundaries, synthesizing what each specialist returns. Coordination is itself a craft, distinct from execution.
...
```
/tkm:agent-team <template> <context> [flags]
```

**Templates:** `tkm:research-tech`, `tkm:takumi`, `tkm:review-code`, `tkm:debug-code`

**Flags:**
- `--devs N` | `--researchers N` | `--reviewers N` | `--debuggers N` -- team size
- `--plan-approval` / `--no-plan-approval` -- plan gate (default: on for takumi)
- `--delegate` -- lead only coordinates, never touches code
- `--worktree` -- use git worktrees for implementation isolation (default: on for takumi)
...
4. Do NOT fall back to subagents. `/tkm:agent-team` MUST use Agent Teams or abort.
...
- For takumi Step 6 MERGE: spawn a dedicated merge teammate instead of lead doing it
...
```
Agent(
  subagent_type: "researcher" | "implementer" | "reviewer" | "debugger" | "tester" | ...,
  description: "short task summary",
  prompt: "full instructions + SK Context Block",
  model: "opus",                    # Required for Agent Teams teammates
  run_in_background: true,          # Non-blocking spawn
  isolation: "worktree"             # Git worktree isolation (takumi devs)
)
```
...
```
SK Context:
- Work dir: {SK_PROJECT_ROOT or CWD}
- Reports: {SK_REPORTS_PATH or "plans/reports/"}
- Plans: {SK_PLANS_PATH or "plans/"}
- Branch: {SK_GIT_BRANCH or current branch}
- Naming: {SK_NAME_PATTERN or "YYMMDD-HHMM"}
- Active plan: {SK_ACTIVE_PLAN or "none"}
...
## ON `/tkm:agent-team research <topic>` [--researchers N]:

*Wraps /tkm:research-tech skill -- scope, gather, analyze, report.*
...
3. **CALL** `TaskCreate` x N -- one per angle:
   ...
   - Description: `Investigate <angle> for topic: <topic>. Save report to: {SK_REPORTS_PATH}/researcher-{N}-{SK_NAME_PATTERN}-{topic-slug}.md. Format: Executive summary, key findings, evidence, recommendations. Mark task completed when done. Send findings summary to lead.`
...
4. **SPAWN** teammates x N via `Agent` tool:
   ...
   - `name: "researcher-{N}"`
   - Prompt: task description + SK Context Block
...
6. **READ** all researcher reports from `{SK_REPORTS_PATH}/`

7. **SYNTHESIZE** into: `{SK_REPORTS_PATH}/research-summary-{SK_NAME_PATTERN}-{topic-slug}.md`
...
11. **JOURNAL**: Run `/tkm:write-journal` to write a concise technical journal entry upon completion

---

## ON `/tkm:agent-team takumi <plan-path-or-description>` [--devs N]:

*Wraps /tkm:takumi skill -- plan, code, test, review, finalize.*
...
3. **CALL** `TaskCreate` x (N + 1) -- N dev tasks + 1 tester tatkm:
   - Dev tasks: include `File ownership: <glob patterns>` -- NO overlap between devs
   - Tester tatkm: `addBlockedBy` all dev task IDs
...
4. **SPAWN** developer teammates x N via `Agent` tool:
   - `subagent_type: "implementer"`, `model: "opus"`
   - `isolation: "worktree"` -- each dev gets isolated git worktree (no file conflicts)
   - `run_in_background: true`
   - `name: "dev-{N}"`
   - Prompt: task description + plan context + SK Context Block
...
7. **DOCS SYNC EVAL** (MANDATORY for takumi -- from /tkm:takumi finalize):
   ```
...
10. **REPORT**: Tell user what was implemented, test results, docs impact.
11. **JOURNAL**: Run `/tkm:write-journal` to write a concise technical journal entry upon completion

---

## ON `/tkm:agent-team review <scope>` [--reviewers N]:

*Wraps /tkm:review-code skill -- scout, review, synthesize with evidence gates.*
...
3. **CALL** `TaskCreate` x N -- one per focus:
   ...
   - Description: `Review <scope> for <focus>. Output severity-rated findings only. Format: [CRITICAL|IMPORTANT|MODERATE] <finding> -- <evidence> -- <recommendation>. No "seems" or "probably" -- concrete evidence only. Save to: {SK_REPORTS_PATH}/reviewer-{N}-{SK_NAME_PATTERN}-{scope-slug}.md. Mark task completed when done.`

4. **SPAWN** reviewers x N via `Agent` tool:
   - `subagent_type: "reviewer"`, `model: "opus"`
   - `run_in_background: true`
   - `name: "reviewer-{N}"`
   - Prompt: task description + SK Context Block
...
6. **SYNTHESIZE** into: `{SK_REPORTS_PATH}/review-{scope-slug}.md`
...
10. **JOURNAL**: Run `/tkm:write-journal` to write a concise technical journal entry upon completion

---

## ON `/tkm:agent-team debug <issue>` [--debuggers N]:

*Wraps /tkm:fix-bug skill -- root-cause-first, adversarial hypotheses, disprove to converge.*
...
7. **WRITE** root cause report: `{SK_REPORTS_PATH}/debug-{issue-slug}.md`
...
| Takumi (4) | ~400K-800K | Highest cost -- code generation |
...
## Worktree Isolation (Takumi Template)
...
> v3.0.0: Agent tool migration, worktree isolation for takumi devs, run_in_background spawning, updated model requirements.
