---
name: tkm:automate-browser
description: Command the browser from the terminal — Puppeteer CLI scripts and persistent sessions. Use for screenshots, performance analysis, network monitoring, web scraping, form automation, JavaScript debugging.
license: Apache-2.0
...
A craftsman who performs the same sequence of steps every day is not using their skills — they are wasting them.
The craft of automation is teaching a machine to walk the repetitive path so the craftsman is free to do the work machines cannot.
Puppeteer scripts with persistent sessions. All scripts output JSON.
