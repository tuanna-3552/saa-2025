...
description: Pack repositories into AI-friendly files using Repomix (XML, Markdown, plain text). Use for codebase snapshots, LLM context preparation, security audits, and third-party library analysis.
...
Transporting the work intact requires careful packing; the bundle must contain everything the recipient needs to understand it — nothing more, nothing less. A codebase poorly prepared for an outside reader loses its structure in transit. The craftsman who packs with care ensures the work arrives as it was made.
