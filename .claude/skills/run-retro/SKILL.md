---
name: tkm:run-retro
description: "Examine the sprint with data, not feelings — git-based retrospective (commits, LOC, hotspots, churn) with derived health indicators and structured markdown or HTML report. Use after sprints, weekly check-ins, or any review period."
license: MIT
...
At the end of a period of work, the guild sits together — not to celebrate, but to examine.
What was built? Where did the work slow? Which commits show the cracks, which show the flow?
The review draws from evidence: objective git metrics, computed health indicators, and findings that can be acted on — no guesswork, no invented data.
