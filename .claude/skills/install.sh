...
install_system_deps() {
    ...
    # ImageMagick (required for generate-slide PDF export)
...
install_node_deps() {
    ...
    npm_packages=(
        ...
        "wrangler:wrangler"
        "repomix:repomix"
        "pptxgenjs:pptxgenjs"   # generate-slide skill: Sun* PPTX generation
    )
    ...
    # automate-browser (puppeteer, sharp, yargs)
    if [ -d "$SCRIPT_DIR/automate-browser/scripts" ] && [ -f "$SCRIPT_DIR/automate-browser/scripts/package.json" ]; then
        print_info "Installing automate-browser dependencies..."
        (cd "$SCRIPT_DIR/automate-browser/scripts" && npm install --quiet)
        print_success "automate-browser dependencies installed"
    fi

    # think-sequential
    if [ -d "$SCRIPT_DIR/think-sequential" ] && [ -f "$SCRIPT_DIR/think-sequential/package.json" ]; then
        print_info "Installing think-sequential dependencies..."
        (cd "$SCRIPT_DIR/think-sequential" && npm install --quiet)
        print_success "think-sequential dependencies installed"
    fi

    # manage-mcp
    if [ -d "$SCRIPT_DIR/manage-mcp/scripts" ] && [ -f "$SCRIPT_DIR/manage-mcp/scripts/package.json" ]; then
        print_info "Installing manage-mcp dependencies..."
        (cd "$SCRIPT_DIR/manage-mcp/scripts" && npm install --quiet)
        print_success "manage-mcp dependencies installed"
    fi
    ...
    # view-plans (gray-matter)
    if [ -d "$SCRIPT_DIR/view-plans" ] && [ -f "$SCRIPT_DIR/view-plans/package.json" ]; then
        print_info "Installing view-plans dependencies..."
        (cd "$SCRIPT_DIR/view-plans" && npm install --quiet)
        print_success "view-plans dependencies installed"
    fi
}
