...
AVAILABLE_STACKS = list(STASK_CONFIG.keys())
...
def search_stack(query, stack, max_results=MAX_RESULTS):
    ...
    if stack not in STASK_CONFIG:
        return {"error": f"Unknown stack: {stack}. Available: {', '.join(AVAILABLE_STACKS)}"}

    filepath = DATA_DIR / STASK_CONFIG[stack]["file"]

    if not filepath.exists():
        return {"error": f"Stack file not found: {filepath}", "stack": stack}

    results = _search_csv(filepath, _STASK_COLS["search_cols"], _STASK_COLS["output_cols"], query, max_results)

    return {
        "domain": "stack",
        "stack": stack,
        "query": query,
        "file": STASK_CONFIG[stack]["file"],
        "count": len(results),
        "results": results
    }
