---
name: tkm:manage-mcp
description: Discover, analyze, and execute MCP server tools, prompts, and resources directly from Claude. Use for MCP integrations, intelligent tool selection, and multi-server management.
...
# Keeping the Guild Tools

A well-maintained collection of instruments is not just stored — it is catalogued, understood, and ready to use without a search.
The tool-keeper knows what each instrument does, when to reach for it, and how to keep the collection from becoming noise that slows every decision.

MCP is an open protocol enabling AI agents to connect to external tools and data sources. This skill discovers, analyzes, and executes MCP capabilities from configured servers without polluting the main context window.
