...
When generating mermaid code blocks, use `/tkm:draw-diagram` skill for v11 syntax rules.
...
For `--slides`: recommend invoking `/tkm:design-ui` for richer style selection.
Must use `/tkm:draw-diagram` for any Mermaid diagrams.
