...
If `/tkm:analyze-media` skill is available and image generation is appropriate, it can be used for hero banners, conceptual illustrations, and decorative accents that establish the page's visual tone.
...
For `--slides` presentation-grade output, consider invoking `/tkm:design-ui` for richer style selection and distinctive font/palette pairing.
