...
description: "Render files in the browser, or generate visual explanations, slides, and diagrams as self-contained HTML. Use when the user says 'show me', 'preview this', 'explain visually', 'make a diagram', or wants to view or render any file or content."
...
Before the piece is finished, it must be shown. Not to the final audience — to the maker, to confirm that the work looks as intended when presented in its proper form.
This skill shows the work: existing files in readable form, or new visual explanations generated from thought.
...
### View Mode
- `/tkm:preview-output <file.md>` - View markdown file in novel-reader UI
- `/tkm:preview-output <directory/>` - Browse directory contents
- `/tkm:preview-output --stop` - Stop running server

### Generation Mode (Markdown)
- `/tkm:preview-output --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/tkm:preview-output --slides <topic>` - Generate presentation slides (one concept per slide)
- `/tkm:preview-output --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/tkm:preview-output --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/tkm:preview-output --html --explain <topic>` - Self-contained HTML explanation
- `/tkm:preview-output --html --slides <topic>` - Magazine-quality HTML slide deck
- `/tkm:preview-output --html --diagram <topic>` - HTML diagram with zoom controls
- `/tkm:preview-output --html --diff [ref]` - Visual diff review
- `/tkm:preview-output --html --plan-review [plan-file]` - Plan vs codebase comparison
...
| Flag without topic | Ask user: "Please provide a topic: `/tkm:preview-output --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/tkm:preview-output --stop` first |
...
Use `/tkm:draw-diagram` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `/tkm:design-ui` for richer style selection
