...
Debug tasks are **separate from** takumi/planning phase tasks.

**When takumi spawns debugger:**
1. Takumi encounters failing tests → creates debug pipeline tasks
2. Debug pipeline executes (assess → collect → analyze → fix → verify)
3. All debug tasks complete → takumi marks phase debugging as done
