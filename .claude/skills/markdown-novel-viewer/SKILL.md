...
description: Present the written work in a calm reading space — serves markdown files in a book-like browser reader via HTTP. Use for plans, specs, journals, documentation, and long-form content that deserves to be read properly.
...
A document is not the work itself — it is how the work is received. Presenting it in proper form, with the right typography, structure, and rendering, is part of completing it. The craftsman who finishes the work but leaves it unreadable has not fully finished.

Background HTTP server rendering markdown files with calm, book-like reading experience.

**Note:** HTML generation mode (`/tkm:preview-output --html ...`) produces self-contained HTML files that open directly in the browser — they do not use this server.
...
Use `/tkm:preview-output` for quick access:
