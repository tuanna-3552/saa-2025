...
Read, don't assume. Use `sk:debug` (systematic-debugging Phase 1).
...
Activate `sk:sequential-thinking` skill. Form hypotheses through structured reasoning.
...
Use `sk:debug` (root-cause-tracing technique). Trace backward:
...
1. Auto-activate `sk:problem-solving` skill
