...
Activate `sk:debug` skill. Activate `sk:sequential-thinking` for structured analysis.
...
Use `reviewer` subagent for quick review.
...
|------|------------------|
| 1 | `sk:scout` (minimal) or direct file read |
| 2 | `sk:debug`, `sk:sequential-thinking` |
| 3 | Parallel `Bash` for verification |
| 4 | `reviewer` subagent |
| 5 | `git-manager` subagent |

**Extra:** `sk:context-engineering` if dealing with AI/LLM code
