...
1. `sk:ui-ux-pro-max` - Design database (ALWAYS FIRST)
2. `sk:ui-ux-pro-max` - Design principles
3. `sk:frontend-design` - Implementation patterns
...
Analyze screenshots/videos with `sk:ai-multimodal` skill.
...
Screenshot + `sk:ai-multimodal` analysis.
...
## Tips
- Use `sk:ai-multimodal` for generating visual assets
