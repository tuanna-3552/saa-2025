...
**Mandatory:** Activate `sk:scout` skill or launch 2-3 `Explore` subagents in parallel:
```
...
2. Activate `sk:debug` skill (systematic-debugging + root-cause-tracing).
3. Activate `sk:sequential-thinking` — structured hypothesis formation.
...
Implement per plan. Use `sk:context-engineering`, `sk:sequential-thinking`, `sk:problem-solving`.
...
- Activate `sk:project-management` for task sync-back, plan status updates, and progress tracking
- Use `doc-writer` subagent for documentation
...
| 2 | `sk:debug`, `sk:sequential-thinking`, parallel `Explore`, (`sk:problem-solving` auto) |
| 3 | `researcher` (runs parallel with steps 1+2) |
| 4 | `sk:brainstorm` |
| 5 | `planner` |
| 6 | `sk:problem-solving`, `sk:sequential-thinking`, `sk:context-engineering` |
| 7 | `tester`, parallel `Bash` verification |
| 8 | `reviewer` |
| 9 | `sk:project-management`, `doc-writer`, `git-manager` |
