---
name: tkm:build-nextjs
description: Master the Next.js framework — App Router, RSC, SSR, ISR, Turborepo monorepos. React apps, server rendering, build optimization, caching strategies, shared dependencies.
license: MIT
...
A craftsman who masters the framework does not fight it — they work within its grain.
Next.js has opinions: Server Components by default, caching by intent, routing by file. The master reads these opinions, understands their reasons, and shapes work that moves with them rather than against them.
