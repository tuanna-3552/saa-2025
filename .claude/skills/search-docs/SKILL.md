...
description: Study the library's own writing before building on it — search API docs and framework documentation via context7. Use for latest library features, API references, GitHub repository analysis, technical documentation lookup.
...
Every craft has a body of accumulated knowledge: specifications written by the makers, edge cases documented by those who discovered them, patterns proven by practitioners who came before.
The craftsman who ignores the guild records invents what already exists and repeats mistakes already solved.

**Script-first** documentation discovery using llms.txt standard. Execute scripts — no manual URL construction needed.
