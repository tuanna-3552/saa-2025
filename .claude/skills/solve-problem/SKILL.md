...
description: Break the pattern when stuck — systematic problem-solving for complexity spirals, innovation blocks, recurring patterns, assumption constraints, simplification cascades, and scale uncertainty.
...
Every craftsman faces the joint that seems impossible: the two surfaces that will not meet, the constraint that seems to rule out every option, the problem that has swallowed three approaches already.
The master does not force the joint. They examine why it is failing, reframe what the joint must accomplish, and find the path that was hidden by the assumption that only one path existed.
