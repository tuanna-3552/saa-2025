...
These phases apply after planning is complete and takumi skill is activated.
Takumi skill handles most of these — this reference documents bootstrap-specific guidance.

## Implementation

Handled by **tkm:takumi** skill. Bootstrap-specific notes:
...
- Asset pipeline: `sk:ai-multimodal` (generate/analyze) → `imagemagick` (crop/resize) → background removal if needed
...
After code review passes. Use `doc-writer` subagent to create/update:
