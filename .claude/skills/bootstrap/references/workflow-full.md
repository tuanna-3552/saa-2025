...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `sk:ai-multimodal` skill generation
3. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `sk:ai-multimodal` skill
5. Screenshot wireframes with `sk:chrome-devtools` → save to `./docs/wireframes/`
...
Activate **sk:plan** skill: `/tkm:create-plan --hard <requirements>`
...
Activate **tkm:takumi** skill: `/tkm:takumi <plan-path>` (interactive mode — review gates at each step)
