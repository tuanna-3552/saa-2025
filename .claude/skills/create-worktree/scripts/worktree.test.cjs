...
const MONOREPO_DIR = '/home/kai/takumi-agent-kit';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'takumi-agent-kit-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from takumi-agent-kit-engineer submodule
  const submodulePath = '/home/kai/takumi-agent-kit/takumi-agent-kit-engineer';
...
test('create with partial project match in monorepo', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'cli' should match 'takumi-agent-kit-cli'
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'takumi-agent-kit-cli', 'Should find takumi-agent-kit-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'takumi-agent-kit' matches multiple projects
  const result = run('create takumi-agent-kit test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
