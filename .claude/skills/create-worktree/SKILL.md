...
When two pieces of work cannot share the same surface without interfering with each other, the craftsman opens a separate bench.
The worktree is that bench: the same workshop, the same tools — but isolated, so each piece of work can proceed without disturbing the other.
