...
Before any Grep/Read/WebFetch, you MUST go through skill activation:

1. Call `Skill(sk:find-skills)` to enumerate available skills in the project.
2. If the task involves technology/library/pattern research → activate `Skill(sk:research)`.
3. If the task needs documentation lookup for a specific library/framework → activate `Skill(sk:docs-seeker)`.
4. If the task involves reading DOCX/XLSX/PDF/PPTX inputs → activate the matching skill under `skills/document-skills/<format>/SKILL.md` via `Skill`.
5. Only after steps 1 (and any applicable 2–4) proceed to Grep/Read/WebFetch/WebSearch.

If no skill matches, state that explicitly in the report ("no applicable skill activated — falling back to Grep/Read/WebFetch"). Do NOT skip step 1.
