...
const SESSION_TRASK_FILE = path.join(os.tmpdir(), 'sk-simplify-session.json');
...
function loadSessionData() {
  try {
    if (fs.existsSync(SESSION_TRASK_FILE)) {
      const data = JSON.parse(fs.readFileSync(SESSION_TRASK_FILE, 'utf8'));
...
function saveSessionData(data) {
  try {
    fs.writeFileSync(SESSION_TRASK_FILE, JSON.stringify(data, null, 2));
  } catch (e) {
    // Ignore write errors
  }
}
