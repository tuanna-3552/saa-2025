...
async function main() {
  ...
  try {
    ...
    if (envFile) {
      // Session & plan config
      writeEnv(envFile, 'SK_SESSION_ID', sessionId || '');
      writeEnv(envFile, 'SK_PLAN_NAMING_FORMAT', config.plan.namingFormat);
      writeEnv(envFile, 'SK_PLAN_DATE_FORMAT', config.plan.dateFormat);
      writeEnv(envFile, 'SK_PLAN_ISSUE_PREFIX', config.plan.issuePrefix || '');
      writeEnv(envFile, 'SK_PLAN_REPORTS_DIR', config.plan.reportsDir);

      // NEW: Resolved naming pattern for DRY file naming in agents
      // Example: "251212-1830-GH-88-{slug}" or "251212-1830-{slug}"
      // Agents use: `{agent-type}-$SK_NAME_PATTERN.md` and substitute {slug}
      writeEnv(envFile, 'SK_NAME_PATTERN', namePattern);

      // Plan resolution
      writeEnv(envFile, 'SK_ACTIVE_PLAN', resolved.resolvedBy === 'session' ? resolved.path : '');
      writeEnv(envFile, 'SK_SUGGESTED_PLAN', resolved.resolvedBy === 'branch' ? resolved.path : '');
      ...
      writeEnv(envFile, 'SK_GIT_ROOT', staticEnv.gitRoot || '');
      writeEnv(envFile, 'SK_REPORTS_PATH', path.join(baseDir, reportsPath));
      writeEnv(envFile, 'SK_DOCS_PATH', path.join(baseDir, config.paths.docs));
      writeEnv(envFile, 'SK_PLANS_PATH', path.join(baseDir, config.paths.plans));
      writeEnv(envFile, 'SK_PROJECT_ROOT', process.cwd());

      // Project detection
      writeEnv(envFile, 'SK_PROJECT_TYPE', detections.type || '');
      writeEnv(envFile, 'SK_PACKAGE_MANAGER', detections.pm || '');
      writeEnv(envFile, 'SK_FRAMEWORK', detections.framework || '');

      // NEW: Static environment info (so other hooks don't need to recompute)
      writeEnv(envFile, 'SK_NODE_VERSION', staticEnv.nodeVersion);
      writeEnv(envFile, 'SK_OS_PLATFORM', staticEnv.osPlatform);
      writeEnv(envFile, 'SK_GIT_BRANCH', staticEnv.gitBranch || '');
      writeEnv(envFile, 'SK_USER', staticEnv.user);
      writeEnv(envFile, 'SK_LOCALE', staticEnv.locale);
      writeEnv(envFile, 'SK_TIMEZONE', staticEnv.timezone);
      writeEnv(envFile, 'SK_CLAUDE_SETTINGS_DIR', staticEnv.claudeSettingsDir);

      // Locale config
      if (config.locale?.thinkingLanguage) {
        writeEnv(envFile, 'SK_THINKING_LANGUAGE', config.locale.thinkingLanguage);
      }
      if (config.locale?.responseLanguage) {
        writeEnv(envFile, 'SK_RESPONSE_LANGUAGE', config.locale.responseLanguage);
      }

      // Plan validation config (for /tkm:create-plan validate, /tkm:create-plan --hard, /tkm:create-plan --parallel)
      const validation = config.plan?.validation || {};
      writeEnv(envFile, 'SK_VALIDATION_MODE', validation.mode || 'prompt');
      writeEnv(envFile, 'SK_VALIDATION_MIN_QUESTIONS', validation.minQuestions || 3);
      writeEnv(envFile, 'SK_VALIDATION_MAX_QUESTIONS', validation.maxQuestions || 8);
      writeEnv(envFile, 'SK_VALIDATION_FOCUS_AREAS', (validation.focusAreas || ['assumptions', 'risks', 'tradeoffs', 'architecture']).join(','));

      // Coding level config (for output style selection)
      const codingLevel = config.codingLevel ?? 5;
      writeEnv(envFile, 'SK_CODING_LEVEL', codingLevel);
      writeEnv(envFile, 'SK_CODING_LEVEL_STYLE', getCodingLevelStyleName(codingLevel));
    ...
    if (envFile && teamInfo) {
      writeEnv(envFile, 'SK_AGENT_TEAM', teamInfo.teamName);
      writeEnv(envFile, 'SK_AGENT_TEAM_MEMBERS', teamInfo.memberCount);
    }
    ...
    if (teamInfo) {
      ...
      console.log(`    Use /tkm:agent-team skill for orchestration templates.`);
    }
