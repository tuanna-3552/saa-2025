...
/**
 * Colorize a single character with minimal ANSI wrapping.
 * Lighter than colorize() — uses bare RESET (no STABLE prefix/suffix overhead).
 * Designed for per-character neon bar coloring where each char needs its own code.
