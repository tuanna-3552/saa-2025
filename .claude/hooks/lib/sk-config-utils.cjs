/**
 * Shared utilities for Takumi Agent Kit hooks
 *
...
const LOCAL_CONFIG_PATH = '.claude/.sk.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.sk.json');
const SESSION_STATE_LOSK_TIMEOUT_MS = 500;
const SESSION_STATE_LOSK_RETRY_MS = 10;
const SESSION_STATE_LOSK_STALE_MS = 5000;
...
function removeStaleSessionStateLock(lockPath, now = Date.now()) {
  try {
    ...
    if (now - stats.mtimeMs < SESSION_STATE_LOSK_STALE_MS) return false;
...
function acquireSessionStateLock(sessionId) {
  ...
  const deadline = Date.now() + SESSION_STATE_LOSK_TIMEOUT_MS;
  ...
  while (Date.now() <= deadline) {
    try {
      ...
      sleepSync(SESSION_STATE_LOSK_RETRY_MS);
    }
  }
 ...
 * @param {Object} config - Takumi Agent Kit config
 ...
 *   2. Global config (~/.claude/.sk.json) - user preferences
 *   3. Local config (./.claude/.sk.json) - project-specific overrides
 *
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.SK_DEBUG) {
      console.error(`[sk-config] Warning: ${validation.error}`);
    }
  }
