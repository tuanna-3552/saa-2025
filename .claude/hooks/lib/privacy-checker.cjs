...
/**
 * Load .sk.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .sk.json (defaults to .claude in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.sk.json')
      : path.join(process.cwd(), '.claude', '.sk.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .sk.json config
