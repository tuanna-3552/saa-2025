...
const DEFAULT_USER_AGENT = 'takumi-agent-kit-engineer/1.0';
const DEFAULT_ELIGIBILITY_CACHE_TTL_MS = 60_000;

function getUsageCachePath() {
  return process.env.SK_USAGE_CACHE_PATH || path.join(os.tmpdir(), 'sk-usage-limits-cache.json');
}

function getQuotaEligibilityCachePath() {
  return process.env.SK_USAGE_ELIGIBILITY_CACHE_PATH || `${getUsageCachePath()}.eligibility`;
}
