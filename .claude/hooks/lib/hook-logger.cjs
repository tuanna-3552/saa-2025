...
const LOSK_FILE = path.join(LOG_DIR, 'hook-log.lock');
const MAX_LINES = 1000;
const TRUNCATE_TO = 500;
const LOSK_TIMEOUT_MS = 250;
const LOSK_RETRY_MS = 10;
...
function withLogLock(fn) {
  ...
  while (Date.now() - startedAt < LOSK_TIMEOUT_MS) {
    let fd;
    try {
      fd = fs.openSync(LOSK_FILE, 'wx');
      try {
        return fn();
      } finally {
        try { fs.closeSync(fd); } catch (_) {}
        try { fs.unlinkSync(LOSK_FILE); } catch (_) {}
      }
    } catch (error) {
