...
const { readSessionState, updateSessionState } = require('./sk-config-utils.cjs');
...
async function refreshStatuslineSnapshot(stdinData) {
  try {
    const sessionId = stdinData.session_id || process.env.SK_SESSION_ID || '';
...
function extractSessionData(stdinData) {
  const data = {
    ...
    branch: process.env.SK_GIT_BRANCH || '',
    plan: process.env.SK_ACTIVE_PLAN || '',
    todos: [], modifiedFiles: []
  };
  const sessionId = stdinData.session_id || process.env.SK_SESSION_ID || '';
