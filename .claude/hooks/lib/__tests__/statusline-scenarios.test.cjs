...
const USAGE_CACHE_PATH = path.join(os.tmpdir(), `sk-usage-limits-cache-statusline-test-${process.pid}.json`);
...
function runStatuslineSync({ payload, cwd = TEST_ROOT, env = {}, inputRaw = null, timeoutMs = 20000 }) {
  ...
  const eligibilityCachePath = env.SK_USAGE_ELIGIBILITY_CACHE_PATH
    || path.join(os.tmpdir(), `sk-usage-eligibility-statusline-test-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.json`);
  const runtimeEnv = {
    ...process.env,
    SK_USAGE_CACHE_PATH: USAGE_CACHE_PATH,
    SK_USAGE_ELIGIBILITY_CACHE_PATH: eligibilityCachePath,
    ...env
  };
  if (!Object.prototype.hasOwnProperty.call(env, 'NO_COLOR')) delete runtimeEnv.NO_COLOR;
  if (!Object.prototype.hasOwnProperty.call(env, 'HOME') && fs.existsSync(path.join(cwd, '.claude', '.sk.json'))) {
    runtimeEnv.HOME = cwd;
  }
...
function withQuotaEligibilityCache(payload, fn) {
  const cachePath = path.join(
    os.tmpdir(),
    `sk-usage-eligibility-statusline-test-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.json`
  );

  try {
    fs.writeFileSync(cachePath, JSON.stringify(payload));
    return fn({ SK_USAGE_ELIGIBILITY_CACHE_PATH: cachePath });
  } finally {
...
function writeSessionStateFile(sessionId, state) {
  const sessionPath = path.join(os.tmpdir(), `sk-session-${sessionId}.json`);
...
async function main() {
    ...
    try {
      withQuotaEligibilityCache({ timestamp: Date.now(), eligible: true, note: 'eligible' }, (env) => {
        withUsageCache({
          ...
          assertContains(result.stdout, '\x1b[31m91%', 'Quota high color should use the configured red tint');
        });
      });
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
  ...
  await test('Quota uses neon defaults when no explicit quota palette is configured', async () => {
    const tmpDir = createTempConfigProject('full', {
      statuslineLayout: {
        lines: [['quota']]
      }
    ...
    try {
      withQuotaEligibilityCache({ timestamp: Date.now(), eligible: true, note: 'eligible' }, (env) => {
        withUsageCache({
          ...
          assertContains(result.stdout, '\x1b[91m91%', 'Quota uses brightRed neon default for high usage without explicit palette');
        });
      });
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
  ...
  await test('Optional stdin timeout forces fallback when enabled', async () => {
    const result = await runStatuslineWithDelayedChunks({
      ...
      delaysMs: [400, 0],
      env: { SK_STATUSLINE_STDIN_TIMEOUT_MS: '100' }
    });
    ...
    withTempConfigProject('full', {}, (tmpDir) => {
      withQuotaEligibilityCache({ timestamp: Date.now(), eligible: true, note: 'eligible' }, (env) => {
        withUsageCache({
          ...
          const { stdout } = result;
          assertMatch(stdout, /5h.*38%/, 'Available usage should show the 5h utilization');
          assertMatch(stdout, /wk.*19%/, 'Available usage should show the weekly utilization');
    ...
    withTempConfigProject('full', {}, (tmpDir) => {
      withQuotaEligibilityCache({ timestamp: Date.now(), eligible: true, note: 'eligible' }, (env) => {
        withUsageCache({
          ...
          assertMatch(result.stdout, /5h.*36%/, 'Legacy raw cache should still render the 5h chip');
          assertMatch(result.stdout, /wk.*18%/, 'Legacy raw cache should still render the weekly chip');
        });
      });
    });
  });
    ...
    try {
      withQuotaEligibilityCache({ timestamp: Date.now(), eligible: true, note: 'eligible' }, (env) => {
        withUsageCache({
          ...
          assertMatch(result.stdout, /5h.*37%/, 'Statusline should keep cosmetic 5h display even when the hook is disabled');
          assertMatch(result.stdout, /wk.*19%/, 'Statusline should keep cosmetic weekly display even when the hook is disabled');
        });
      });
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    ...
    try {
      withQuotaEligibilityCache({ timestamp: Date.now(), eligible: true, note: 'eligible' }, (env) => {
        withUsageCache({
          ...
          assertTrue(!result.stdout.includes('5h'), 'Quota toggle should hide the 5h chip');
          assertTrue(!result.stdout.includes(' wk '), 'Quota toggle should hide the weekly chip');
          assertContains(result.stdout, 'TAKUMI', 'Other statusline content should still render');
        });
      });
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    ...
    withTempConfigProject('full', {}, (tmpDir) => {
      withQuotaEligibilityCache({ timestamp: Date.now(), eligible: true, note: 'eligible' }, (env) => {
        withUsageCache({
          ...
          assertTrue(!result.stdout.includes('5h'), 'Third-party runtime should auto-hide the 5h chip');
          assertTrue(!result.stdout.includes(' wk '), 'Third-party runtime should auto-hide the weekly chip');
          assertContains(result.stdout, 'TAKUMI', 'Other statusline content should still render');
        });
      });
    });
