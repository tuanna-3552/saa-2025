...
test('getGitInfo timeout does not cache null result', () => {
  ...
  const cachePath = path.join(os.tmpdir(), `sk-git-cache-${hash}.json`);
  const originalPath = process.env.PATH;
  const originalTimeout = process.env.SK_GIT_TIMEOUT_MS;
  ...
  try {
    ...
    process.env.SK_GIT_TIMEOUT_MS = '50';
    ...
    if (originalTimeout == null) delete process.env.SK_GIT_TIMEOUT_MS;
    else process.env.SK_GIT_TIMEOUT_MS = originalTimeout;
