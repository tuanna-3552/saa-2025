 ...
 *
 * Uses 'ignore' package for .skignore parsing and path matching.
...
// Default patterns if .skignore doesn't exist or is empty
...
function readPatternsFromFile(filePath) {
  ...
  try {
    ...
    console.error('WARN: Failed to read .skignore:', error.message);
    return [];
  }
}

/**
 * Load patterns from the shipped .skignore plus an optional project override.
 * Falls back to DEFAULT_PATTERNS if the shipped file doesn't exist or is empty.
 *
 * @param {string} skignorePath - Path to shipped/global .skignore file
 * @param {string} [projectSkignorePath] - Optional project-local .skignore path
 * @returns {string[]} Array of patterns
 */
function loadPatterns(skignorePath, projectSkignorePath) {
  const shippedPatterns = readPatternsFromFile(skignorePath);
  const projectPatterns = readPatternsFromFile(projectSkignorePath);
 ...
 *
 * @param {string[]} patterns - Array of patterns from .skignore
 ...
 *
 * @param {string[]} originalPatterns - Original patterns from .skignore
