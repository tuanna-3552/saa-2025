...
try {
  ...
  const os = require('os');
  const { isHookEnabled } = require('./lib/sk-config-utils.cjs');
...
/**
 * Build SK stack context from environment variables
...
function buildSkContext() {
  ...
  if (env.SK_REPORTS_PATH) ctx.push(`Reports: ${env.SK_REPORTS_PATH}`);
  if (env.SK_PLANS_PATH) ctx.push(`Plans: ${env.SK_PLANS_PATH}`);
  if (env.SK_PROJECT_ROOT) ctx.push(`Project: ${env.SK_PROJECT_ROOT}`);
  if (env.SK_NAME_PATTERN) ctx.push(`Naming: ${env.SK_NAME_PATTERN}`);
  if (env.SK_GIT_BRANCH) ctx.push(`Branch: ${env.SK_GIT_BRANCH}`);
  if (env.SK_ACTIVE_PLAN) ctx.push(`Active plan: ${env.SK_ACTIVE_PLAN}`);
