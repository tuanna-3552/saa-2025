...
describe('descriptive-name.cjs', () => {
  ...
  describe('Hook disable functionality', () => {
    ...
    it('exits silently when disabled via .sk.json', async () => {
      // Create .claude/.sk.json with hook disabled
      const claudeDir = path.join(tempDir, '.claude');
      fs.mkdirSync(claudeDir, { recursive: true });
      fs.writeFileSync(
        path.join(claudeDir, '.sk.json'),
      ...
      fs.copyFileSync(
        path.join(__dirname, '..', 'lib', 'sk-config-utils.cjs'),
        path.join(libDir, 'sk-config-utils.cjs')
      );
