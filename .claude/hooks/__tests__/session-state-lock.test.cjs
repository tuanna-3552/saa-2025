#!/usr/bin/env node
/**
 * Tests for session temp-state locking in sk-config-utils.cjs
...
const LIB_PATH = path.join(__dirname, '..', 'lib', 'sk-config-utils.cjs');
...
describe('sk-config-utils session state locking', () => {
