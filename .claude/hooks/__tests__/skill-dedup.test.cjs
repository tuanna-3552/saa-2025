...
describe('listSkillNames', () => {
  ...
  it('returns skill names that have SKILL.md', () => {
    createSkill(globalDir, 'takumi');
    createSkill(globalDir, 'brainstorm');
    const result = listSkillNames(globalDir);
    assert.deepStrictEqual(result.sort(), ['brainstorm', 'takumi']);
  });
...
describe('findOverlaps', () => {
  ...
  it('finds overlapping names', () => {
    assert.deepStrictEqual(
      findOverlaps(['takumi', 'brainstorm', 'git'], ['takumi', 'seo', 'brainstorm']),
      ['takumi', 'brainstorm']
    );
  });

  it('returns empty array when global is empty', () => {
    assert.deepStrictEqual(findOverlaps([], ['takumi']), []);
  });
...
describe('SKIP_DIRS', () => {
  ...
  it('does not contain regular skill names', () => {
    assert.ok(!SKIP_DIRS.has('takumi'));
...
describe('handleSessionStart', () => {
  ...
  it('shadows overlapping local skills', () => {
    createSkill(paths.globalDir, 'takumi', '# Takumi v2.1.1 (engineer)');
    createSkill(paths.globalDir, 'brainstorm', '# Brainstorm (engineer)');
    createSkill(paths.localDir, 'takumi', '# Takumi v2.0.0 (marketing)');
    ...
    assert.deepStrictEqual(result.shadowed.sort(), ['brainstorm', 'takumi']);
    assert.ok(!skillExists(paths.localDir, 'takumi'));
    assert.ok(!skillExists(paths.localDir, 'brainstorm'));
    assert.ok(skillExists(paths.shadowedDir, 'takumi'));
  ...
  it('writes a manifest file with shadowed skill names', () => {
    ...
    assert.deepStrictEqual(manifest.skills, ['takumi']);
  ...
  it('recovers from crashed previous session before shadowing', () => {
    ...
    createSkill(paths.shadowedDir, 'takumi', '# Takumi orphaned');

    const result = handleSessionStart(paths);

    // Should have restored takumi first, then re-shadowed it
    assert.ok(skillExists(paths.shadowedDir, 'takumi'));
    assert.ok(!skillExists(paths.localDir, 'takumi'));
  });
});
...
describe('handleSessionEnd', () => {
  ...
  it('restores shadowed skills from manifest', () => {
    ...
    createSkill(paths.localDir, 'seo');
    fs.mkdirSync(paths.shadowedDir, { recursive: true });
    createSkill(paths.shadowedDir, 'takumi', '# Takumi (marketing)');
    ...
    fs.writeFileSync(paths.manifestFile, JSON.stringify({
      ...
      skills: ['takumi', 'brainstorm']
    }));

    const result = handleSessionEnd(paths);

    assert.deepStrictEqual(result.restored.sort(), ['brainstorm', 'takumi']);
    assert.ok(skillExists(paths.localDir, 'takumi'));
    assert.ok(skillExists(paths.localDir, 'brainstorm'));
    assert.ok(skillExists(paths.localDir, 'seo'));
  ...
  it('cleans up .shadowed directory after restore', () => {
    fs.mkdirSync(paths.shadowedDir, { recursive: true });
    createSkill(paths.shadowedDir, 'takumi');
    fs.writeFileSync(paths.manifestFile, JSON.stringify({ skills: ['takumi'] }));
  ...
  it('handles corrupt manifest by restoring orphaned skills', () => {
    ...
    assert.deepStrictEqual(result.restored, ['takumi']);
    assert.ok(skillExists(paths.localDir, 'takumi'));
    assert.ok(!fs.existsSync(paths.shadowedDir));
  });
  ...
  it('skips restore if local skill already exists (no overwrite)', () => {
    createSkill(paths.localDir, 'takumi', '# Takumi local version');
    fs.mkdirSync(paths.shadowedDir, { recursive: true });
    createSkill(paths.shadowedDir, 'takumi', '# Takumi shadowed version');
    fs.writeFileSync(paths.manifestFile, JSON.stringify({ skills: ['takumi'] }));
    ...
    const content = fs.readFileSync(path.join(paths.localDir, 'takumi', 'SKILL.md'), 'utf8');
    assert.strictEqual(content, '# Takumi local version');
  });
});
...
describe('full session cycle', () => {
  ...
  it('SessionStart -> SessionEnd restores original state', () => {
    ...
    assert.deepStrictEqual(endResult.restored.sort(), ['brainstorm', 'takumi']);
    ...
    // Content preserved
    const takumiContent = fs.readFileSync(path.join(paths.localDir, 'takumi', 'SKILL.md'), 'utf8');
    assert.strictEqual(takumiContent, '# Takumi v2.0.0');
  });
  ...
  it('handles crash recovery: SessionStart after crashed session', () => {
    createSkill(paths.globalDir, 'takumi');
    createSkill(paths.localDir, 'takumi', '# Original local');
    ...
    // takumi should still be shadowed (recovered then re-shadowed)
    assert.ok(!skillExists(paths.localDir, 'takumi'));
    assert.ok(skillExists(paths.shadowedDir, 'takumi'));
...
describe('edge cases', () => {
  ...
  it('preserves skill directory contents (not just SKILL.md)', () => {
    ...
    fs.writeFileSync(path.join(paths.localDir, 'takumi', 'README.md'), '# Readme');
    fs.mkdirSync(path.join(paths.localDir, 'takumi', 'scripts'));
    fs.writeFileSync(path.join(paths.localDir, 'takumi', 'scripts', 'run.sh'), '#!/bin/bash');

    handleSessionStart(paths);
    handleSessionEnd(paths);

    // All files preserved
    assert.ok(fs.existsSync(path.join(paths.localDir, 'takumi', 'SKILL.md')));
    assert.ok(fs.existsSync(path.join(paths.localDir, 'takumi', 'README.md')));
    assert.ok(fs.existsSync(path.join(paths.localDir, 'takumi', 'scripts', 'run.sh')));
  });
