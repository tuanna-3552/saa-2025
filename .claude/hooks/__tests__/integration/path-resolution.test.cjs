...
describe('Issue #327: Path Resolution Integration', () => {
  ...
  describe('Monorepo/Worktree Scenarios', () => {
    ...
    it('scout-block uses payload.cwd to discover a git-root .claude/.skignore', async () => {
      ...
      fs.writeFileSync(
        path.join(tempHome, '.claude', '.sk.json'),
      ...
      fs.writeFileSync(path.join(projectRoot, '.claude', '.skignore'), '!build\n');
      ...
      try {
        ...
        assert.ok(
          blockResult.stderr.includes(path.join(projectRoot, '.claude', '.skignore')),
