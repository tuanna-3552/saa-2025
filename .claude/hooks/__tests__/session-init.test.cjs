...
describe('session-init.cjs', () => {
  ...
  describe('Session Context Output', () => {
    ...
    it('restores orphaned shadowed skills on startup', async () => {
      ...
      const shadowedSkillDir = path.join(tempDir, '.claude', 'skills', '.shadowed', 'takumi');
      const restoredSkillDir = path.join(tempDir, '.claude', 'skills', 'takumi');

      fs.mkdirSync(shadowedSkillDir, { recursive: true });
      fs.writeFileSync(path.join(shadowedSkillDir, 'SKILL.md'), '# Takumi shadowed copy\n');
