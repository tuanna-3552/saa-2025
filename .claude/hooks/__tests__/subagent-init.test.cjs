...
describe('subagent-init.cjs', () => {
  ...
  describe('Basic Functionality', () => {
    ...
    it('includes agent type and ID in output', async () => {
      ...
      assert.ok(context.includes('reviewer'), 'Should include agent type');
  ...
  describe('Issue #540: ck plan CLI injection for plan-aware agents', () => {
    ...
    it('injects ck plan CLI section for plan-aware agent types', async () => {
      const planAwareTypes = ['planner', 'project-manager', 'code-simplifier', 'brainstormer', 'reviewer', 'implementer'];
  ...
  describe('Monorepo/Submodule Scenarios', () => {
    ...
    it('outputs SK_DEBUG info when enabled', async () => {
      const result = await runHook({
        agent_type: 'test-agent',
        agent_id: 'debug-test',
        cwd: process.cwd()
      ...
      if (process.env.SK_DEBUG || result.stderr.includes('effectiveCwd')) {
        assert.ok(
