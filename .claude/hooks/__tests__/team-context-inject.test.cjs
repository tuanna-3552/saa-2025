 ...
 * - SK stack context building from environment variables
...
describe('team-context-inject.cjs', () => {
  ...
  describe('SK Stack Context Building', () => {

    it('includes SK context when environment variables are set', async () => {
      const tmpDir = path.join(os.tmpdir(), 'team-inject-sk-' + Date.now());
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        ...
        const result = await runHook({
          ...
          env: {
            HOME: tmpDir,
            SK_REPORTS_PATH: '/project/plans/reports',
            SK_PLANS_PATH: '/project/plans',
            SK_PROJECT_ROOT: '/project',
            SK_GIT_BRANCH: 'main'
          }
        });
        ...
        assert.ok(context.includes('SK Context'), 'Should have SK Context section');
    ...
    it('always includes commit convention in SK context', async () => {
    ...
    it('only includes SK context lines when env vars exist (fail-open)', async () => {
      const tmpDir = path.join(os.tmpdir(), 'team-inject-empty-sk-' + Date.now());
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        createTestTeam(tmpDir, 'team-a');

        // Run with no SK_* env vars
        const result = await runHook({
          agent_id: 'alice@team-a'
        }, {
          env: { HOME: tmpDir }
        ...
        // Should not have empty SK Context section
        // Commit convention is always present, so SK Context should appear
  ...
  describe('Error Handling & Robustness', () => {
    ...
    it('logs to stderr only when SK_DEBUG is enabled', async () => {
