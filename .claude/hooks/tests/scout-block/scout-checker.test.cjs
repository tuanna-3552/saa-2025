 ...
 * Uses real .skignore fixtures to validate end-to-end behavior.
 *
...
const DEFAULT_OPTS = {
  ...
  skignorePath: path.join(FIXTURES_DIR, 'skignore-default.txt'),
...
function createProjectRoot({ rootPatterns, nestedPatterns } = {}) {
  ...
  if (rootPatterns) {
    ...
    fs.writeFileSync(path.join(repoRoot, '.claude', '.skignore'), `${rootPatterns.join('\n')}\n`);
  }
  if (nestedPatterns) {
    fs.mkdirSync(path.join(repoRoot, 'pkg', '.claude'), { recursive: true });
    fs.writeFileSync(path.join(repoRoot, 'pkg', '.claude', '.skignore'), `${nestedPatterns.join('\n')}\n`);
  }
  return repoRoot;
}
...
describe('checkScoutBlock - fail-open behavior', () => {
  ...
  it('uses defaults when skignorePath missing', () => {
    const r = checkScoutBlock({ toolName: 'Bash', toolInput: { command: 'ls node_modules' }, options: { skignorePath: '/nonexistent/.skignore' } });
    assert.ok(r.blocked, 'Should still block using DEFAULT_PATTERNS');
  });
});

describe('checkScoutBlock - custom .skignore', () => {
  const customOpts = {
    ...DEFAULT_OPTS,
    skignorePath: path.join(FIXTURES_DIR, 'skignore-custom.txt')
  };
...
describe('checkScoutBlock - negation .skignore', () => {
  const negOpts = {
    ...DEFAULT_OPTS,
    skignorePath: path.join(FIXTURES_DIR, 'skignore-negation.txt')
  };
...
describe('checkScoutBlock - project-local .claude/.skignore discovery', () => {
  it('allows build paths via the git-root .claude/.skignore override', () => {
  ...
  it('still blocks node_modules and points to the git-root .claude/.skignore file', () => {
    const projectRoot = createProjectRoot({ rootPatterns: ['!build'] });
    const r = checkScoutBlock({
      toolName: 'Read',
    ...
    assert.ok(r.blocked);
    assert.strictEqual(r.configPath, path.join(projectRoot, '.claude', '.skignore'));
  });
  ...
  it('ignores nested .claude/.skignore files below the git root', () => {
